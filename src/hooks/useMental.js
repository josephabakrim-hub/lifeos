...
export function useMental() {
  ...
  // Week score — only counts logs from Monday of the current week up to today
  function getWeekScore() {
    ...
    const moodLogs    = weekLogs.filter(l => l.morning?.mood)
    const stressLogs  = weekLogs.filter(l => l.evening?.stress)
    const clarityLogs = weekLogs.filter(l => l.evening?.clarity)
    ...
    const moodScore       = avgMood    ? ((avgMood / 5) * 100)                    : 0
    const stressScore     = avgStress  ? ((1 - ((avgStress - 1) / 4)) * 100)      : 0
    const clarityScore    = avgClarity ? ((avgClarity / 5) * 100)                 : 0
