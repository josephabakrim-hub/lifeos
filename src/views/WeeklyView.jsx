...
const CATEGORY_COLORS = {
  ...
  Learning: '#f59e0b', Mental: '#a855f7', Social: '#ec4899', Other: '#9898b0',
}

function ParetoAudit({ activeGoals, weeklyGoals, paretoAnswer, onSave }) {
  const [open,   setOpen]   = useState(false)
  const [answer, setAnswer] = useState(paretoAnswer || '')
  const [picked, setPicked] = useState([]) // goal ids marked as the vital 20%
  const [saved,  setSaved]  = useState(false)

  // Sync when parent data loads
  useEffect(() => { setAnswer(paretoAnswer || '') }, [paretoAnswer])
  ...
  function handleSave() {
    onSave({ paretoAnswer: answer, paretoPicked: picked })
    setSaved(true)
    setTimeout(() => setSaved(false), 2000)
  }

  // Combine life goals + this week's goals into one ranked list for the audit
  const lifeGoalItems = activeGoals.map(g => ({
    id:       g.id,
    label:    g.title,
    category: g.category,
  ...
  const weeklyItems = weeklyGoals
    .filter(g => g.text)
    .map((g, i) => ({
      id:       `week-${i}`,
      label:    g.text,
      category: null,
      progress: g.done ? 100 : 0,
      source:   'week',
    }))

  const allItems = [...lifeGoalItems, ...weeklyItems]

  return (
    <div className="card" style={{ marginBottom: 16, border: '1px solid rgba(245,158,11,0.25)' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 14, color: 'var(--text)' }}>80/20 Audit — Pareto Principle</div>
            <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 1 }}>
              {open ? 'Which 20% of your goals drive 80% of the results?' : 'Optional · activate when you need focus clarity'}
            </div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {!open && paretoAnswer && (
            <span className="badge badge-amber" style={{ fontSize: 10 }}>✓ Done this week</span>
          )}
      ...
      {open && (
        <div style={{ marginTop: 16 }}>
          ...
          {/* Context */}
          <div style={{ padding: '10px 14px', borderRadius: 8, background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.2)', marginBottom: 16, fontSize: 13, color: 'var(--text2)', lineHeight: 1.7 }}>
            <strong style={{ color: '#f59e0b' }}>The 80/20 Rule:</strong> 80% of your results come from 20% of your efforts. Most of what you do is low-leverage. A few actions are the ones that actually move the needle. Identify them — then protect them at all costs this week.
          </div>

          {/* Goal selector */}
          {allItems.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>
                Mark your vital 20% — the goals that matter most this week
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {allItems.map(item => {
                  const isMarked = picked.includes(item.id)
                  const color    = item.category ? (CATEGORY_COLORS[item.category] || 'var(--accent)') : 'var(--accent)'
                  return (
                    <div
                      key={item.id}
                      ...
                      style={{
                        ...
                        background: isMarked ? `${color}15` : 'var(--bg3)',
                        border: `1px solid ${isMarked ? color : 'var(--border)'}`,
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        border: `2px solid ${isMarked ? color : 'var(--border2)'}`,
                        background: isMarked ? color : 'transparent',
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 600, color: isMarked ? 'var(--text)' : 'var(--text2)' }}>
                          {item.label}
                        </div>
                        <div style={{ display: 'flex', gap: 8, marginTop: 2, alignItems: 'center' }}>
                          {item.source === 'life' && item.category && (
                            <span style={{ fontSize: 10, fontWeight: 600, color, padding: '1px 6px', borderRadius: 10, background: `${color}20` }}>
                              {item.category}
                            </span>
                          )}
                          {item.source === 'week' && (
                            <span style={{ fontSize: 10, color: 'var(--text3)' }}>This week's goal</span>
                          )}
                          {item.source === 'life' && (
                            <span style={{ fontSize: 10, color: 'var(--text3)' }}>{item.progress}% complete</span>
                          )}
                        </div>
                      </div>

                      {isMarked && (
                        <span style={{ fontSize: 11, fontWeight: 700, color, flexShrink: 0 }}>Vital 20%</span>
                      )}
                    </div>
                  )
                })}
              ...
              {picked.length > 0 && (
                <div style={{ marginTop: 10, padding: '10px 14px', borderRadius: 8, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.25)', fontSize: 13, color: 'var(--text2)', lineHeight: 1.6 }}>
                  ⚡ <strong style={{ color: '#f59e0b' }}>Your vital 20%:</strong> {picked.length} goal{picked.length > 1 ? 's' : ''} selected. These get your best hours, best energy, best focus this week. Everything else is secondary.
                </div>
              )}
            </div>
          )}
          ...
          <div className="form-group" style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text2)' }}>
              What is the ONE action this week that drives 80% of progress across your goals?
            </label>
            <textarea
              value={answer}
              onChange={e => setAnswer(e.target.value)}
              rows={3}
              placeholder="e.g. Execute only A+ setups every day this week. If I do this one thing consistently, everything else follows."
          ...
          {/* Leverage tip */}
          {answer.length > 20 && (
            <div style={{ marginBottom: 12, padding: '10px 14px', borderRadius: 8, background: 'var(--bg3)', fontSize: 12, color: 'var(--text3)', lineHeight: 1.6 }}>
              💡 <strong>Apply it:</strong> Block time for this in your calendar before anything else. Protect it like a trading session — no interruptions, no exceptions.
            </div>
          )}
...
export default function WeeklyView({ plans, loading, getCurrentPlan, savePlan, savePlanForWeek, updatePlan, deletePlan, getWeekScore, activeGoals = [] }) {
  ...
  const [saving,          setSaving]          = useState(false)
  const [saved,           setSaved]           = useState(false)
  const [paretoAnswer,    setParetoAnswer]    = useState('')
  ...
  useEffect(() => {
    ...
    setParetoAnswer(currentPlan.paretoAnswer || '')
  ...
  async function handleSave() {
    setSaving(true)
    await savePlan({ goals, reflection, sundayPlanDone, fridayReviewDone, deepWorkHours: deepWorkVal, paretoAnswer })
    setSaving(false)
    setSaved(true)
    setTimeout(() => setSaved(false), 2000)
  }
  ...
  return (
    <div className="fade-in">
      ...
      <ParetoAudit
        ...
        weeklyGoals={goals}
        paretoAnswer={paretoAnswer}
        onSave={({ paretoAnswer: pa }) => {
          setParetoAnswer(pa)
          savePlan({ goals, reflection, sundayPlanDone, fridayReviewDone, deepWorkHours: deepWorkVal, paretoAnswer: pa })
        }}
      />
